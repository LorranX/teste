...
const jadibot = async (reply, client, id) => {
  ...
  conn.browserDescription = [ 'Conexão Secundaria', '', '3.0' ];
  conn.on('qr', async qr =>{
    let bot = await qrcode.toDataURL(qr, { scale: 8 });
    let buffer = new Buffer.from(bot.replace('data:image/png;base64,', ''), 'base64');
    bot = await client.sendMessage(id,buffer,MessageType.image,{caption:'Pro bot logar no seu numero leia o QR code acima\n*eu enviarei um QR code a cada 30 segundos ate que alguem se conecte'});
    setTimeout(() => {
